		...
		<template #additional="group">
			...
			>
				<div
						...
						class="foxui-carousel__controls"
				>
					<template
					...
					>
						...
						>
							...
							>
								<foxui-btn v-bind="controlProps(item, index, group)"/>
							</slot>
						</slot>
					</template>
				</div>
...
<script
		...
		setup
>
	import { FoxuiBtn, FoxuiProgressLinear, FoxuiWindow } from '@foxui/components'

	import { useLocale, useProps, useVModel } from '@foxui/composables'

	import { DENSITY, MDI_ICONS, SIZES } from '@foxui/enums'

	import type { ICarouselProps, IGroupProvide } from '@foxui/interfaces'

	import type { TFoxuiWindow } from "@foxui/types"
	...
	const foxuiWindowRef = ref<TFoxuiWindow>()

	let slideTimeout = -1

	const startTimeout = () => {
		if (!props.cycle || !foxuiWindowRef.value) return

		slideTimeout = window.setTimeout(foxuiWindowRef.value.group.next, +props.interval > 0 ? +props.interval : 6000)
	}
	...
	const windowProps = computed(() => {
		return foxuiWindowRef.value?.filterProps(props, ['modelValue'])
	})
	...
	const controlProps = (item: {
		...
		return Object.assign({},
				item,
				{
					...
					'aria-label': t('foxui.carousel.ariaLabel.delimiter', index + 1, group.items.value.length),
					active: group.isSelected(item.id),
					class: [
						'foxui-carousel__controls-item'
					],
	...
	const carouselClasses = computed(() => {
		return [
			'foxui-carousel',
			{
				'foxui-carousel--hide-delimiter-background': props.hideDelimiterBackground,
				'foxui-carousel--vertical-delimiters': props.verticalDelimiters
			},
			props.class
		]
	})
