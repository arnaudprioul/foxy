<template>
    ...
    <span
		    v-if="isClickable"
		    key="overlay"
		    class="foxui-chip__overlay"
    />
		<span
				key="underlay"
				class="foxui-chip__underlay"
		/>

		<template v-if="hasFilter">
			<foxui-expand-x key="filter">
				<div
						v-show="isSelected"
						class="foxui-chip__filter"
				>
					<slot
							name="filter"
							v-bind="{filterIcon}"
					>
						<foxui-icon :icon="filterIcon"/>
					</slot>
				</div>
			</foxui-expand-x>
		</template>
...
<script
		...
		setup
>
	import { FoxuiAvatar, FoxuiExpandX, FoxuiIcon } from '@foxui/components'
	...
	import type { IChipProps } from '@foxui/interfaces'
	...
	const group = useGroupItem(props, FOXUI_CHIP_GROUP_KEY, false)
	...
	const chipClasses = computed(() => {
		return [
			'foxui-chip',
			{
				'foxui-chip--disabled': props.disabled,
				'foxui-chip--label': props.label,
				'foxui-chip--link': isClickable.value,
				'foxui-chip--filter': hasFilter.value,
				'foxui-chip--pill': props.pill
			},
			borderClasses.value,
...
>
	.foxui-chip {
		...
		&--density-default {
			--foxui-chip---density: 0px;
		}

		&--density-compact {
			--foxui-chip---density: -8px;
		}
