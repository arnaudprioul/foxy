<template>
	...
	>
		<template v-if="SUPPORTS_EYE_DROPPER">
			<div
					key="eyeDropper"
					class="foxui-color-picker-preview__eye-dropper"
			>
				<foxui-btn
		...
		<div class="foxui-color-picker-preview__dot">
			<div :style="{ background: HSVtoCSS(colorHsv ?? COLOR_NULL) }"/>
		</div>

		<div class="foxui-color-picker-preview__sliders">
			<foxui-slider-field
					:disabled="disabled"
					:max="360"
					...
					class="foxui-color-picker-preview__track foxui-color-picker-preview__hue"
					hide-details
			...
			<template v-if="!hideAlpha">
				<foxui-slider-field
						...
						class="foxui-color-picker-preview__track foxui-color-picker-preview__alpha"
						hide-details
...
<script
		...
		setup
>
	import { FoxuiBtn, FoxuiSliderField } from "@foxui/components"

	import { useProps, useVModel } from "@foxui/composables"

	import { COLOR_NULL, SUPPORTS_EYE_DROPPER } from "@foxui/consts"

	import { DENSITY, MDI_ICONS } from "@foxui/enums"

	import type { IColorPickerPreviewProps } from "@foxui/interfaces"

	import { consoleWarn, HSVtoCSS, parseColor, RGBtoHSV } from "@foxui/utils"
	...
	const colorPickerPreviewClasses = computed(() => {
		return [
			...
			{
				'foxui-color-picker-preview--hide-alpha': props.hideAlpha
			},
			props.class
		]
	})
...
>
	.foxui-color-picker-preview {
		...
		&__alpha {
			...
			:deep(.foxui-slider-field-track__background) {
				background-color: transparent;
				background-image: linear-gradient(to right, transparent, var(--foxui-color-picker-color-hsv));
