	...
	>
		...
		>
			<template v-if="column.key === 'data-table-group'">
				...
				>
					<foxui-data-table-column-cell class="foxui-data-table-group-header-row__column">
						<foxui-btn
								:icon="groupIcon"
								size="small"
								variant="text"
...
>
	...
	import { useGroupBy, useHeaders, useProps, useSelection } from "@foxui/composables"
	import { MDI_ICONS } from "@foxui/enums"

	import type { IDataTableGroupHeaderRowProps } from "@foxui/interfaces"
	...
	const dataTableGroupHeaderRowClasses = computed(() => {
		return [
			'foxui-data-table-group-header-row',
			props.class
		]
	})
	const dataTableGroupHeaderRowStyles = computed(() => {
		return [
			{
				'--foxui-data-table-group-header-row--depth': props.item.depth
			},
			props.style
		] as StyleValue
	})
