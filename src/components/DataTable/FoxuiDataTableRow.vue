					...
					<template #default>
						...
						<template v-else>
							<template v-if="mobile">
								<div class="foxui-data-table-row__column-title">
									<slot
							...
							<div class="foxui-data-table-row__column-value">
								<slot
...
>
	...
	import { FoxuiBtn, FoxuiCheckboxBtn, FoxuiDataTableColumnCell } from '@foxui/components'

	import { useCell, useDisplay, useExpanded, useHeaders, useProps, useSelection, useSort } from '@foxui/composables'

	import { MDI_ICONS, SIZES } from '@foxui/enums'

	import type { IDataTableHeaderCellColumnSlot, IDataTableItemKey, IDataTableRowProps } from '@foxui/interfaces'

	import { getCurrentInstance, getObjectValueByPath } from '@foxui/utils'
	...
	const {displayClasses, mobile} = useDisplay(props, 'foxui-data-table-row')
	...
	const dataTableColumnCellClasses = (key: string) => {
		return [
			'foxui-data-table-row__column',
			{
				'foxui-data-table-row__column--expanded-row': key === 'data-table-expand',
				'foxui-data-table-row__column--select-row': key === 'data-table-select'
			}
		]
	}
	...
	const dataTableRowClasses = computed(() => {
		return [
			'foxui-data-table-row',
			{
				'foxui-data-table-row--clickable': !!(Object.prototype.hasOwnProperty.call(vm.vnode.props, 'onClick') || Object.prototype.hasOwnProperty.call(vm.vnode.props, 'onContextmenu') || Object.prototype.hasOwnProperty.call(vm.vnode.props, 'onDblclick'))
			},
	...
	.foxui-data-table-row {
		...
		&--mobile {
			...
			> .foxui-data-table-column-cell {
				...
				min-height: var(--foxui-data-table-row--mobile__column);
...
<style>
	:root {
		--foxui-data-table-row--mobile__column: 52px;
	}
</style>
