<template>
	<template v-if="loading">
		<tr
				key="loading"
				class="foxui-data-table-rows foxui-data-table-rows--loading"
		>
	...
	<template v-else-if="!(items && items.length) && !hideNoData">
		<tr
				key="no-data"
				class="foxui-data-table-rows foxui-data-table-rows--no-data"
		>
			<td :colspan="columns.length">
				<slot name="no-data">
					{{ t(noDataText) }}
...
<script
		...
		setup
>
	import { FoxuiDataTableGroupHeaderRow, FoxuiDataTableRow } from '@foxui/components'
	...
	import { getPrefixedEventHandlers } from '@foxui/utils'
	...
	const props = withDefaults(defineProps<IDataTableRowsProps>(), {
		loadingText: 'foxui.dataIterator.loadingText',
