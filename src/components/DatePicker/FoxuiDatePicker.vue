<template>
	...
	>
		...
		<slot name="default">
			<foxui-date-picker-controls
					ref="foxuiDatePickerControlsRef"
					:text="text"
			...
			<foxui-fade>
				<template v-if="viewModeIsMonths">
					<foxui-date-picker-months
							key="date-picker-months"
							ref="foxuiDatePickerMonthsRef"
							v-model:month="month"
							:max="maxDate"
							:min="minDate"
							:year="year"
				...
				<template v-else-if="viewModeIsYears">
					<foxui-date-picker-years
							key="date-picker-years"
							ref="foxuiDatePickerYearsRef"
							v-model:year="year"
							:max="maxDate"
							:min="minDate"
...
>
	import {
		...
		FoxuiDatePickerMonths,
		FoxuiDatePickerYears,
		FoxuiFade,
		FoxuiPicker,
		FoxuiReverseTranslatePicker,
		FoxuiTranslatePicker
	} from "@foxui/components"

	import { useDate, useLocale, useProps, useVModel } from "@foxui/composables"

	import { CALENDAR_STRATEGY, DATE_MODE } from "@foxui/enums"

	import type { IDatePickerProps } from "@foxui/interfaces"

	import type {
		TFoxuiDatePickerControls,
		TFoxuiDatePickerHeader,
		TFoxuiDatePickerMonth,
		TFoxuiDatePickerMonths,
		TFoxuiDatePickerYears,
		TFoxuiPicker
	} from "@foxui/types"
	...
	const props = withDefaults(defineProps<IDatePickerProps>(), {
		...
		title: 'foxui.datePicker.title',
		header: 'foxui.datePicker.header'
	})
	...
	const header = computed(() => {
		if ((props.multiple || props.range) && model.value?.length > 1) {
			return t('foxui.datePicker.itemsSelected', model.value?.length)
		}
	...
	const headerTransition = computed(() => {
		return {
			component: isReversing.value ? FoxuiReverseTranslatePicker : FoxuiTranslatePicker
		}
	})
	...
	const foxuiPickerRef = ref<TFoxuiPicker>()
	const foxuiDatePickerControlsRef = ref<TFoxuiDatePickerControls>()
	const foxuiDatePickerHeaderRef = ref<TFoxuiDatePickerHeader>()
	const foxuiDatePickerMonthRef = ref<TFoxuiDatePickerMonth>()
	const foxuiDatePickerMonthsRef = ref<TFoxuiDatePickerMonths>()
	const foxuiDatePickerYearsRef = ref<TFoxuiDatePickerYears>()

	const pickerProps = computed(() => {
		return foxuiPickerRef.value?.filterProps(props, ['class', 'style', 'title', 'id'])
	})
	const datePickerControlsProps = computed(() => {
		return foxuiDatePickerControlsRef.value?.filterProps(props, ['class', 'style', 'id', 'text', 'disabled'])
	})
	const datePickerHeaderProps = computed(() => {
		return foxuiDatePickerHeaderRef.value?.filterProps(props, ['class', 'style', 'header', 'id'])
	})
	const datePickerMonthProps = computed(() => {
		return foxuiDatePickerMonthRef.value?.filterProps(props, ['class', 'style', 'id', 'date', 'min', 'max', 'year', 'month'])
	})
	const datePickerMonthsProps = computed(() => {
		return foxuiDatePickerMonthsRef.value?.filterProps(props, ['class', 'style', 'id', 'month', 'min', 'max', 'year'])
	})
	const datePickerYearsProps = computed(() => {
		return foxuiDatePickerYearsRef.value?.filterProps(props, ['class', 'style', 'id', 'year', 'min', 'max'])
	})
	...
	const datePickerClasses = computed(() => {
		return [
			'foxui-date-picker',
			`foxui-date-picker--${viewMode.value}`,
			{
				'foxui-date-picker--show-week': props.showWeek
			},
			props.class
		]
	})
...
>
	.foxui-date-picker {
		...
		&--year {
			:deep(.foxui-date-picker-controls) {
				.foxui-date-picker-controls__mode-btn {
