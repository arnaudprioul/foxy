					...
					<template #default="{class: fieldSlotClass, ...fieldSlotProps}">
						...
						>
							...
							<template
									...
									:key="index"
							>
								<div class="foxui-file-field__selection">
									...
									<template v-else>
				            <span class="foxui-file-field__selection-text">
				              <slot name="selection">
				                {{ filename }}
				              </slot>
				              <span
						              v-if="multiple && (index < model.length - 1)"
						              class="foxui-file-field__selection-comma"
				              >
				                {{ divider }}
				              </span>
				            </span>
...
<script
		...
		setup
>
	import { FoxuiChip, FoxuiCounter, FoxuiField, FoxuiInput } from '@foxui/components'

	import { useAdjacent, useAdjacentInner, useFocus, useLocale, useProps, useVModel } from '@foxui/composables'

	import { DENSITY, MDI_ICONS } from '@foxui/enums'

	import type { IFileFieldProps } from '@foxui/interfaces'

	import type { TFoxuiInput } from '@foxui/types'

	import { filterInputAttrs, forwardRefs, humanReadableFileSize, wrapInArray } from '@foxui/utils'
	...
	const props = withDefaults(defineProps<IFileFieldProps>(), {
		...
		rounded: true,
		divider: ',',
		counterSizeString: 'foxui.fileInput.counterSize',
		counterString: 'foxui.fileInput.counter'
	})
	...
	const foxuiFieldRef = ref<TFoxuiInput>()
	...
	const fieldProps = computed(() => {
		return foxuiFieldRef.value?.filterProps(props, ['class', 'style', 'id', 'active', 'dirty', 'disabled', 'focused', 'error'])
	})
	...
	const fileFieldClasses = computed(() => {
		return [
			'foxui-file-field',
			{
				'foxui-file-field--chips': props.chips
			},
			props.class
		]
	})
