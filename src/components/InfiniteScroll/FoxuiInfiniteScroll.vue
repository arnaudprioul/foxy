<template>
	...
	>
		<div class="foxui-infinite-scroll__side">
		...
		<template v-if="rootEl && hasStartIntersect && isIntersectMode">
			<foxui-infinite-scroll-intersect
...
<script
		...
		setup
>
	import { FoxuiBtn, FoxuiInfiniteScrollIntersect, FoxuiProgress } from '@foxui/components'

	import { useBothColor, useDimension, useLocale, useProps } from '@foxui/composables'
	...
	import type { IInfiniteScrollProps } from '@foxui/interfaces'

	import type { TInfiniteScrollSide, TInfiniteScrollStatus } from '@foxui/types'
	...
	const props = withDefaults(defineProps<IInfiniteScrollProps>(), {
		...
		tag: 'div',
		loadMoreText: 'foxui.infiniteScroll.loadMore',
		emptyText: 'foxui.infiniteScroll.empty'
	})
	...
	const infiniteScrollClasses = computed(() => {
		return [
			'foxui-infinite-scroll',
			`foxui-infinite-scroll--${props.direction}`,
			{
				'foxui-infinite-scroll--start': hasStartIntersect.value,
				'foxui-infinite-scroll--end': hasEndIntersect.value
			},
			props.class
		]
	})
