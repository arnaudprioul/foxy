<template>
	...
	>
		...
		<template #prependInner>
			<div
					v-if="!hideControls"
					class="foxui-number-field__control"
			>
				<template v-if="split">
					<slot
							name="increment"
					...
					<foxui-divider :direction="DIRECTION.VERTICAL"/>
				</template>
			</div>
...
<script
		...
		setup
>
	import { FoxuiBtn, FoxuiDivider, FoxuiTextField } from "@foxui/components"

	import { useAdjacentInner, useFocus, useForm, useHold, useProps, useVModel } from "@foxui/composables"

	import { DIRECTION, MDI_ICONS, TEXT_FIELD_TYPE } from "@foxui/enums"

	import type { INumberFieldProps } from "@foxui/interfaces"

	import type { TFoxuiTextField } from "@foxui/types"

	import { clamp, forwardRefs, omit } from "@foxui/utils"
	...
	const clampModel = () => {
		if (controlsDisabled.value) return
		if (!foxuiTextFieldRef.value) return

		const actualText = foxuiTextFieldRef.value.value
	...
	const textFieldProps = computed(() => {
		return foxuiTextFieldRef.value?.filterProps(props, ['modelValue', 'class', 'style', 'validationValue'])
	})
	...
	const numberFieldClasses = computed(() => {
		return [
			'foxui-number-field',
			{
				'foxui-number-field--hide-input': props.hideInput,
				'foxui-number-field--inset': props.inset,
				'foxui-number-field--split': props.split,
				'foxui-number-field--hide-controls': props.hideControls,
				'foxui-number-field--reverse': props.reverse
			},
			props.class
		]
	})
	...
	defineExpose(forwardRefs({filterProps}, foxuiTextFieldRef))
...
>
	.foxui-number-field {
		...
		:deep(.foxui-field) {
			.foxui-field__prepend-inner {
				&:has(.foxui-number-field__control) {
					> .foxui-icon {
		...
		:deep(.foxui-field--appended) {
			--foxui-field---padding-end: 0;
		}
