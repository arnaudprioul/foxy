<template>
	...
	>
		...
		<template
				...
				#prependInner="{}"
		>
			<template v-if="split && !hideControls">
				<div class="foxy-number-field__control">
					<slot
							name="increment"
...
>
	...
	import { useAdjacentInner, useFocus, useForm, useHold, useProps, useSlots, useVModel } from "@foxy/composables"
	...
	import { computed, nextTick, onMounted, ref, shallowRef, StyleValue, watch, watchEffect } from "vue"
	...
	const hasAppendInner = computed(() => {
		return hasSlot('appendInner') || hasControl('decrement') || hasControl('increment')
	})
	const hasPrependInner = computed(() => {
		return hasSlot('appendInner') || hasControl('increment')
	})
	const hasControl = (type: string) => {
		return hasSlot(type)
	}
