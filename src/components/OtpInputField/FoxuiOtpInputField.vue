<template>
	...
	>
		<div
				...
				class="foxui-otp-input-field__content"
		>
			<template
					v-for="(_, i) in fields"
					:key="i"
			>
				<template v-if="divider && i !== 0">
					<span class="foxui-otp-input-field__divider">
						{{ divider }}
					</span>
				</template>
				...
				>
					...
					<template #default>
						<input
								...
								class="foxui-otp-input-field__field"
								@blur="handleBlur"
			...
			<input
					:value="model.join('')"
					class="foxui-otp-input-field__input"
					type="hidden"
					v-bind="{ ...inputAttrs }"
			/>
			...
			<foxui-overlay
					:model-value="!!loading"
					contained
					content-class="foxui-otp-input-field__loader"
					persistent
			>
				<template #loader>
					<slot name="loader">
...
>
	...
	import { FoxuiField, FoxuiOverlay, FoxuiProgress } from "@foxui/components"

	import { useDimension, useFocus, useLocale, useProps, useVModel } from "@foxui/composables"

	import { OTP_INPUT_FIELD_TYPE, PROGRESS_TYPE } from "@foxui/enums"

	import type { IOtpInputFieldProps } from "@foxui/interfaces"

	import type { TFoxuiField } from "@foxui/types"

	import { filterInputAttrs, focusChild } from "@foxui/utils"
	...
	const foxuiFieldRef = ref<Array<TFoxuiField>>([])
	...
	const fieldProps = (index: number) => {
		return foxuiFieldRef.value?.[index]?.filterProps(props, ['class', 'style', 'id', 'label'])
	}
	...
	const otpInputFieldClasses = computed(() => {
		return [
			...
			{
				'foxui-otp-input-field--divided': !!props.divider
			},
			props.class
		]
	})
