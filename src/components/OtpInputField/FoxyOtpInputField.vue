<template>
  ...
  >
    ...
    >
      <template v-for="(_, i) in fields">
        <template v-if="divider && i !== 0">
          <span class="foxy-otp-input-field__divider">{{ divider }}</span>
        </template>
        ...
        >
          ...
          <template #default>
            <input
                ...
                class="foxy-otp-input-field__field"
                @blur="handleBlur"
      ...
      <input
          :value="model.join('')"
          class="foxy-otp-input-field__input"
          type="hidden"
          v-bind="{ ...inputAttrs }"
      />
      ...
      <foxy-overlay
          :model-value="!!loading"
          contained
          content-class="foxy-otp-input-field__loader"
          persistent
      >
        <template #loader>
          <slot name="loader">
...
>
  ...
  const foxyFieldRef = ref<Array<TFoxyField>>([])
  ...
  const fieldProps = (index: number) => {
    return foxyFieldRef.value?.[index]?.filterProps(props, ['class', 'style', 'id', 'label'])
  }
  ...
  const otpInputFieldClasses = computed(() => {
    return [
      ...
      {
        'foxy-otp-input-field--divided': !!props.divider
      },
      props.class
    ]
  })
