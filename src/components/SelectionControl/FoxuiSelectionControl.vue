<template>
	...
	>
		<div
				...
				class="foxui-selection-control__wrapper"
		>
			<slot
					name="default"
			...
			<div
					v-ripple="rippleProp"
					class="foxui-selection-control__input"
			>
				<slot
						name="input"
		...
		<div class="foxui-selection-control__label">
			<slot name="label">
				<foxui-label
						:for="id"
						:required="required"
...
<script
		...
		setup
>
	import { FoxuiIcon, FoxuiLabel } from '@foxui/components'

	import { useProps, useSelectionControl } from '@foxui/composables'

	import { vRipple } from '@foxui/directives'

	import type { ISelectionControlProps } from "@foxui/interfaces"

	import { filterInputAttrs, forwardRefs, getUid, matchesSelector } from '@foxui/utils'
	...
	const selectionControlClasses = computed(() => {
		return [
			...
			{
				'foxui-selection-control--dirty': model.value,
				'foxui-selection-control--disabled': props.disabled,
				'foxui-selection-control--error': props.error,
				'foxui-selection-control--focused': isFocused.value,
				'foxui-selection-control--focus-visible': isFocusVisible.value,
				'foxui-selection-control--inline': props.inline
			},
...
>
	.foxui-selection-control {
		...
		&__wrapper {
			width: calc(40px + 1.5 * var(--foxui-selection-control--density));
			height: calc(40px + 1.5 * var(--foxui-selection-control--density));
			display: inline-flex;
			align-items: center;
			position: relative;
		...
		&--density-default {
			--foxui-selection-control--density: 0px;
		}

		&--density-compact {
			--foxui-selection-control--density: -8px;
		}
	}
</style>
