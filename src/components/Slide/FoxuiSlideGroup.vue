<template>
	...
	>
		...
		<div
				key="container"
				ref="containerRef"
				class="foxui-slide-group__container"
				@scroll="handleScroll"
		>
			<div
					ref="contentRef"
					class="foxui-slide-group__content"
...
<script
		...
		setup
>
	import { FoxuiFade, FoxuiIcon } from "@foxui/components"
	import {
		useBorder,
		useDisplay,
		useGoTo,
	...
	import { FOXUI_SLIDE_GROUP_KEY, IN_BROWSER } from "@foxui/consts"

	import { DIRECTION, MDI_ICONS } from "@foxui/enums"

	import type { IGoToOptions, ISlideGroupProps } from "@foxui/interfaces"
	import {
	...
	const props = withDefaults(defineProps<ISlideGroupProps>(), {
		...
		selectedClass: 'foxui-slide-group-item--active'
	})
	...
	const group = useGroup(props, FOXUI_SLIDE_GROUP_KEY)
	...
	const slideGroupClasses = computed(() => {
		return [
			'foxui-slide-group',
			{
				'foxui-slide-group--vertical': !isHorizontal.value,
				'foxui-slide-group--has-affixes': hasAffixes.value,
				'foxui-slide-group--is-overflowing': isOverflowing.value
			},
	...
	const slideGroupPrevClasses = computed(() => {
		return [
			'foxui-slide-group__prev',
			{
				'foxui-slide-group__prev--disabled': !hasPrev.value
			}
		]
	})
	const slideGroupNextClasses = computed(() => {
		return [
			'foxui-slide-group__next',
			{
				'foxui-slide-group__next--disabled': !hasPrev.value
			}
		]
	})
