...
>
	...
	const group = useGroup(props, FOXY_SLIDE_GROUP_KEY)
	...
	const slideGroupClasses = computed(() => {
		return [
			'foxy-slide-group',
			{
				'foxy-slide-group--vertical': !isHorizontal.value,
				'foxy-slide-group--has-affixes': hasAffixes.value,
				'foxy-slide-group--is-overflowing': isOverflowing.value
			},
	...
	const slideGroupPrevClasses = computed(() => {
		return [
			'foxy-slide-group__prev',
			{
				'foxy-slide-group__prev--disabled': !hasPrev.value
			}
		]
	})
	const slideGroupNextClasses = computed(() => {
		return [
			'foxy-slide-group__next',
			{
