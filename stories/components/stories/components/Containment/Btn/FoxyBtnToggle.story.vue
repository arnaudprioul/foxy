  ...
  >
    <Variant title="Default">
          ...
          <foxy-row :align="ALIGN.CENTER" :justify="JUSTIFY.CENTER">
            <foxy-col cols="auto">
              <foxy-btn-toggle v-bind="state" v-model="state.modelValue" @update:model-value="logEvent('update:modelValue', $event)">
                <template #item="{item}">
                  <foxy-btn v-bind="item" @group:selected="logEvent('group:selected', $event)"/>
                </template>
              </foxy-btn-toggle>
            </foxy-col>
          </foxy-row>
      ...
      <template #controls>
        ...
        <hst-checkbox v-model="state.mandatory" title="Required"/>

        <hst-checkbox v-model="state.disabled" title="Disabled"/>

        <hst-checkbox v-model="state.divided" title="Divided"/>
      </template>
    </Variant>
  </Story>
</template>
...
<script lang="ts" setup>
  import { FoxyBtn, FoxyBtnToggle, FoxyCol, FoxyContainer, FoxyRow } from '@foxy/components'
  ...
  const state: { [key: string]: any } = reactive({
    modelValue: 'justify',
    items: [
      {value: 'left', icon: MDI_ICONS.FORMAT_ALIGN_LEFT},
      {value: 'center', icon: MDI_ICONS.FORMAT_ALIGN_CENTER},
      {value: 'right', icon: MDI_ICONS.FORMAT_ALIGN_RIGHT},
      {value: 'justify', icon: MDI_ICONS.FORMAT_ALIGN_JUSTIFY}
    ]
  })
</script>
<docs lang="md">
